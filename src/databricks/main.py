...
def add_args(parser: argparse.ArgumentParser):
    ...
    parser.add_argument('--example_arg', nargs='?')

    return parser

def execute_job(job_path: str, example_arg: str):
    ...
    '''
    job_module = job_path.rstrip('.py')
    module_path = 'jobs/' + job_module
    module_path = module_path.replace('/', '.')
    module = import_module(module_path)
    execute_function = getattr(module, 'execute')
    execute_function(example_arg)

arg_parser = argparse.ArgumentParser()
arg_parser = add_args(arg_parser)
arguments = arg_parser.parse_args()
execute_job(job_path=arguments.job_path, example_arg=arguments.example_arg)