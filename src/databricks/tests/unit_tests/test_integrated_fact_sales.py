...
from jobs.target.FactSales import transform
...
def test_integrated_fact_sales_transforms(expected_df_cleansed_sales: DataFrame, expected_df_integrated_fact_sales: DataFrame) -> None:
    ...
    actual_df_integrated_fact_sales: DataFrame = transform(expected_df_cleansed_sales) # Apply imported transform logic to the expected cleansed sales DF here
